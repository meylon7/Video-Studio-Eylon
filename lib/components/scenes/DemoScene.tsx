 ...
 *
 * content: {
 *   demoType?: 'browser' | 'terminal' | 'raw';
 *   videoFile?: string;
 *   label?: string;
 *   caption?: string;
 * }
 */
export const DemoScene: React.FC<SceneProps> = ({ content }) => {
  const frame = useCurrentFrame();
  const theme = useTheme();
  const isRTL = useIsRTL();

  const demoType = content.demoType || content.type || 'raw';
    ...
    >
      ...
      >
        ...
        >
          ...
          {(demoType === 'browser' || demoType === 'terminal') && (
            <div
              style={{
                display: 'flex',
                alignItems: 'center',
                gap: 8,
                padding: '12px 16px',
                background: demoType === 'terminal' ? '#1a1a1a' : '#2d2d2d',
