 ...
 * }
 */
export const ProblemScene: React.FC<SceneProps> = ({ content }) => {
  ...
  const problems = (content.problems || []) as Array<{ icon: string; text: string }>;
  const codeExample = content.codeExample as string[] | undefined;
  ...
  const codeBlockX = interpolate(frame, [15, 45], [-50 * dir, 0], {
    extrapolateRight: 'clamp',
  });

  // Problem icons animation (right side)
  const iconAnimations = problems.map((_, i) => {
  ...
  return (
    <AbsoluteFill style={{ padding: 80 }}>
      ...
      >
        {headlineParts[0]}
        {headlineParts.length > 1 && <span style={{ color: '#FF6B6B' }}>...</span>}
      </h2>
      ...
      >
        {/* Code Block */}
        {codeExample && codeExample.length > 0 && (
          <div
            style={{
